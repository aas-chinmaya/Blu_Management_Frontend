...
export default function EmployeeTasksList() {
  ...
  const employeeTasksFromStore = useSelector(selectAllTaskListByEmployeeId);

const employeeTasks = useMemo(
  () => employeeTasksFromStore ?? [],
  [employeeTasksFromStore]
);
  ...
  const taskStats = useMemo(() => ({
    ...
    high: processedTasks.filter(t => t.priority === "High").length,
    medium: processedTasks.filter(t => t.priority === "Medium").length,
    low: processedTasks.filter(t => t.priority === "Low").length,
  }), [processedTasks]);
  ...
  const filteredAndSortedTasks = useMemo(() => {
    ...
    result.sort((a, b) => {
      switch (sortBy) {
        ...
        case "projectName-asc": return (a.projectName || "").localeCompare(b.projectName || "");
        case "projectName-desc": return (b.projectName || "").localeCompare(a.projectName || "");
    ...
    return result;
  }, [processedTasks, searchTerm, selectedStatus, selectedPriority, sortBy]);
  ...
  const hasActiveFilters = searchTerm || selectedStatus !== "all" || selectedPriority !== "all" || sortBy !== "id-desc";
                  ...
                  {[
                    {
                      ...
                      content: (
                        <div className="relative">
                          ...
                          <Input
                            placeholder="Title, project, assigned by, or ID..."
                            value={searchTerm}
                                  ...
                                  {opt.value !== "all" && (
                                    <Badge variant="secondary" className="ml-3 text-xs">
                                      {opt.value === "Pending" ? taskStats.Pending
                                        : opt.value === "In Progress" ? taskStats.inProgress : taskStats.completed}
                                    </Badge>
                                  )}
                                </div>
                              </SelectItem>
                    ...
                    },
                    {
                      ...
                      content: (
                        <Select value={sortBy} onValueChange={setSortBy}>
                          <SelectTrigger className="w-full bg-white border-gray-300 focus:border-teal-500 focus:ring-2 focus:ring-teal-200 h-10">
                            <SelectValue placeholder="ID (High to Low)" />
                          </SelectTrigger>
                          <SelectContent>
                            {sortOptions.map(opt => (
                        ...
                        {paginatedTasks.length === 0 ? (
                          <TableRow>
                            <TableCell colSpan={5} className="text-center py-20">
                              ...
                              <p className="text-xl font-medium text-gray-600">No tasks assigned</p>
                              <p className="text-gray-500">You re all caught up!</p>
                            </TableCell>
                          </TableRow>
                        ) : (
                          paginatedTasks.map((task) => (
                            <Tooltip.Root key={task._id}>
                              <Tooltip.Trigger asChild>
                                <TableRow
