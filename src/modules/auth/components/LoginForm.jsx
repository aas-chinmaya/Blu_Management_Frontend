...
import { Eye, EyeOff, CornerUpLeft, Shield, LogInIcon } from "lucide-react";
import { toast } from "sonner";
...
const LoginForm = () => {
  ...
  const handleOtpSubmit = async () => {
    ...
    try {
      ...
      if (response?.message) {
        toast.success(response.message); // e.g. "Login successful"
      }
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      if (response?.message) {
        toast.success(response.message); // e.g. "Fresh OTP sent"
      }
    ...
    <div className="flex justify-center space-x-3 mb-6">
      {otp.map((digit, index) => (
        <Input
          ...
          className="w-14 h-16 text-center text-2xl font-bold border-3 border-blue-300 rounded-2xl focus:border-blue-500 focus:ring-4 focus:ring-blue-200 shadow-lg transition-all hover:scale-105"
          autoFocus={index === 0 && mode === "otp"}
        />
      ))}
    </div>
  );
  ...
  return (
    <Card className="max-w-md w-full rounded-3xl shadow-2xl p-8 border-0 relative overflow-hidden">
      <div className="absolute top-0 left-0 w-full h-2 bg-blue-400"></div>
      ...
      {mode === "login" && (
        <>
          <CardHeader className="mb-8 text-center">
            <CardTitle className="text-4xl font-black bg-gradient-to-r from-blue-600 to-blue-700 bg-clip-text text-transparent">
              Welcome Back!
            </CardTitle>
            <p className="text-gray-700 mt-3 text-lg font-medium">
              Step into your productivity!
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-6">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="user@aas.technology"
                className="h-14 rounded-2xl shadow-lg focus:ring-4 focus:ring-blue-300 border-2 border-blue-200 font-medium placeholder-blue-500"
              />
            </div>
            ...
            <div className="space-y-2 relative">
              ...
              <Input
                ...
                placeholder="Enter your password"
                className="h-14 rounded-2xl shadow-lg focus:ring-4 focus:ring-blue-300 border-2 border-blue-200 font-medium placeholder-blue-500 pr-14"
              />
              <button
                type="button"
                onClick={() => setShowPassword(!showPassword)}
                className="absolute right-4 top-11 text-blue-600"
                tabIndex={-1}
              >
                {showPassword ? <EyeOff size={22} /> : <Eye size={22} />}
              </button>
            ...
            <div className="flex justify-between items-center mt-6">
              <button
                type="button"
                onClick={handleBackToHome}
                className="flex items-center gap-1 text-sm font-semibold text-gray-700 hover:text-blue-700 underline"
              >
            ...
            <Button
              ...
              className="w-full bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 font-bold rounded-2xl h-14 text-lg text-white shadow-xl hover:scale-105"
            >
              {isLoginLoading ? (
      ...
      {mode === "otp" && (
        <>
          <CardHeader className="mb-6 text-center">
            <CardTitle className="text-3xl font-black bg-gradient-to-r from-blue-600 to-blue-700 bg-clip-text text-transparent">
              Security Check!
            </CardTitle>
            <p className="text-gray-700 mt-3">
              OTP sent to{" "}
          ...
          <div className="flex justify-between items-center mb-8 gap-4">
            ...
            <Button
              ...
              className="bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 font-bold rounded-2xl shadow-xl text-white hover:scale-105 disabled:opacity-50"
            >
              {isVerifyLoading ? (
                <div className="flex items-center gap-2">
                  <div className="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent"></div>
